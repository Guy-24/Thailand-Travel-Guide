...
export default function Navbar({
    ...
    if (showReviewPopup && user) {
      const fetchMyReviews = async () => {
        ...
        try {
          ...
          const res = await fetch(`${apiUrl}/api/review/my_reviews`, {
  ...
  function InfoModal({ isOpen, onClose, title, items, emptyMessage, type }) {
          ...
          <div className="p-4 overflow-y-auto flex-1">
            {items.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {/* รูปแบบการแสดงผลข้อมูลใน Popup */}
                {items.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-3 border rounded-xl bg-gray-50 flex flex-col gap-1"
                  >
                    {type === "favorite" ? (
                      ...
                      <>
                        <div className="flex justify-between items-center">
                          {/* แสดงชื่อสถานที่ที่เพิ่งเพิ่มเข้าไปใน Schema */}
                          <span className="font-medium text-emerald-700">
                            {item.place_name}
                          </span>
                          {/* แสดงดาว */}
                          <span className="text-amber-400 text-sm font-semibold bg-white px-2 py-0.5 rounded shadow-sm border border-amber-100">
                            ⭐ {item.rating}
                          </span>
                        </div>
    ...
    <>
      <nav className="bg-white shadow-sm sticky top-0 z-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          {/* Nav links */}
          <div className="hidden md:flex ml-auto items-center gap-1 text-sm font-medium text-gray-600">
            <a
              href="#"
              onClick={(e) => {
                e.preventDefault();
      ...
      <InfoModal
        ...
        items={favoriteItems}
        emptyMessage="ยังไม่ได้กดถูกใจสถานที่ใดเลย"
        type="favorite"
      />
      <InfoModal
        isOpen={showReviewPopup}
        onClose={() => setShowReviewPopup(false)}
        title="ประวัติการรีวิวของคุณ"
        items={myReviews}
        emptyMessage={
          isLoadingReviews
            ? "กำลังโหลดข้อมูล..."
            : "คุณยังไม่ได้รีวิวสถานที่ใดเลย"
        }
        type="review"
      />
    </>
